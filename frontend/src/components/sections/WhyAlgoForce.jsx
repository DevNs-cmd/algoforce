...
const WhyAlgoForce = () => {
  ...
  const visibleAdvantages = showAllAdvantages ? advantages : advantages.slice(0, 3);
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <div className="text-center mb-12 px-2">
            <h2 className="text-[12px] font-bold uppercase tracking-[0.4em] text-purple-600 mb-4">Business value</h2>
          ...
          {/* Core Advantages */}
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mb-8 max-w-6xl mx-auto">
            <AnimatePresence>
              {visibleAdvantages.map((adv, i) => (
                <motion.div
                  ...
                  className="p-6 md:p-8 rounded-[24px] bg-gray-50 border border-gray-100 hover:shadow-lg hover:shadow-purple-500/5 transition-all group"
                >
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12">
            <button
              ...
              className="px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider text-purple-600 hover:text-purple-700 border border-purple-200 bg-purple-50 hover:bg-purple-100 transition-all inline-flex items-center gap-2"
            >
              {showAllAdvantages ? 'Show Top Value Drivers' : 'View All 6 Value Drivers'}
              {showAllAdvantages ? <FaChevronUp size={10} /> : <FaChevronDown size={10} />}
            </button>

            <button
              onClick={() => setShowComparison(!showComparison)}
              className="px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider text-gray-700 hover:text-black border border-gray-200 bg-white hover:bg-gray-50 transition-all inline-flex items-center gap-2 shadow-sm"
            >
              {showComparison ? 'Hide Comparison Matrix' : 'View Model Comparison Matrix'}
              {showComparison ? <FaChevronUp size={10} /> : <FaChevronDown size={10} />}
            </button>
          </div>

          {/* Comparison Summary Table - Collapsible */}
          <AnimatePresence>
            {showComparison && (
              <motion.div
